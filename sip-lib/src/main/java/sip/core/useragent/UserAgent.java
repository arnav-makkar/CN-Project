...
import sip.Logger;
import sip.XmlConfig;
import sip.media.AbstractSoundManager;
import sip.media.Echo;
...
public class UserAgent {
    ...
    public final static String CONFIG_FILE = "conf" + File.separator + "config.xml";
    ...
    private UserAgent(SipListener sipListener, Config config, String peersHome,
        ...
        if (config.getLocalInetAddress() == null) {
            throw new RuntimeException("Local IP address not configured. Please set ipAddress in config file or ensure network is available.");
        }
        if (config.getUserPart() == null || config.getUserPart().trim().isEmpty()) {
            throw new RuntimeException("User part not configured. Please set userPart in config file.");
        }
        if (config.getDomain() == null || config.getDomain().trim().isEmpty()) {
            throw new RuntimeException("Domain not configured. Please set domain in config file.");
        }
    ...
    public SipRequest sendMessage(String toUri, String messageText, byte[] attachment, 
            String attachmentName, String contentType) throws SipUriSyntaxException {
        return uac.sendMessage(toUri, messageText, attachment, attachmentName, contentType);
    }
